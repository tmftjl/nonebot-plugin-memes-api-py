...
usage_help_cmd = on_command("表情帮助", block=True, priority=11)
info_cmd = on_command(
    "表情详情", aliases={"表情帮助", "表情示例"}, block=True, priority=11
)
search_cmd = on_command("表情搜索", block=True, priority=11)

add_whitelist_cmd = on_command("添加保护", block=True, priority=11, permission=SUPERUSER)
remove_whitelist_cmd = on_command(
    "移除保护", block=True, priority=11, permission=SUPERUSER
)
protect_meme_cmd = on_command("保护表情", block=True, priority=11, permission=SUPERUSER)
unprotect_meme_cmd = on_command(
    "取消保护表情", block=True, priority=11, permission=SUPERUSER
)
protection_list_cmd = on_command(
    "保护列表", block=True, priority=11, permission=SUPERUSER
)

statistics_cmd = on_command(
    "表情调用统计",
    aliases={"表情使用统计"},
    block=True,
    priority=11,
...
block_cmd = on_command("禁用表情", block=True, priority=11)
unblock_cmd = on_command("启用表情", block=True, priority=11)
...
refresh_cmd = on_command(
    "更新表情", aliases={"刷新表情"}, block=True, priority=3, permission=SUPERUSER
)

meme_msg_matcher = on_message(rule=meme_trigger_rule(), block=False, priority=3)
