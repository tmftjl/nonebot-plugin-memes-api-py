...
- **白名单保护**：当有人 @ 白名单用户触发攻击类表情时，自动反转目标。支持热更新，使用命令动态管理
...
超级用户可以通过以下命令管理白名单保护功能：

- `添加保护@用户` 或 `添加保护<QQ号>` - 将用户添加到保护白名单（无需空格）
- `移除保护@用户` 或 `移除保护<QQ号>` - 从保护白名单移除用户（无需空格）
- `保护表情<表情名>` - 将表情添加到保护列表（如：打、砍头等，无需空格）
- `取消保护表情<表情名>` - 从保护列表移除表情（无需空格）
- `保护列表` - 查看当前的保护配置

**工作原理**：
- 当有人 @ 白名单中的用户触发保护表情时：
  - 单图表情（如砍头）：将白名单用户替换为发起者
  - 双图表情（如打）：反转两个图的顺序
...
配置会实时保存到 `./data/protection_config.yaml`，无需重启机器人
